...
def repos(username):
    ...
    url = '{base_url}/users/{username}/'.format(base_url=base_url, username=username)
...
def repo_exists(reponame, username):
    ...
    try:
        url = '{base_url}/repositories/{username}/{reponame}/'.format(base_url=base_url, username=username, reponame=reponame)
...
def create_repo(reponame, username, password):
    ...
    if not repo_exists(reponame, username):
        # Somehow authentication with urllib2 is not working. So using this ugly approach.
        cmd = 'curl -d "name={reponame}" -u {username}:{password} {base_url}/repositories/'
        cmd = cmd.format(reponame=reponame, username=username, password=password, base_url=base_url)
